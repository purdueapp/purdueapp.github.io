$(document).ready(function(){if(window.location.search.indexOf("hideChrome")>0||window.location.href.indexOf("nintendo-switch")>0){var i=document.cookie.split(";");
for(var j=0;
j<i.length;
j++){var f=i[j];
var h=f.indexOf("=");
var g=h>-1?f.substr(0,h):f;
if(g){document.cookie=g+"=;  domain="+location.hostname+"; path=/; expires="+new Date(0).toUTCString();
document.cookie=g+"=;   path=/; expires="+new Date(0).toUTCString()
...
$("#language-switcher a").click(function(b){b.preventDefault();
var a=$(b.target);
createCookie("language_url",a.attr("href"));
...
}}function createCookieOnConsent(a,b){if(typeof(mscc)==="undefined"||mscc.hasConsent()){document.cookie=a+"="+b+"; path=/; SameSite=None; Secure"
...
}if(typeof(mscc)==="undefined"||mscc.hasConsent()){var b=document.createElement("script");
...
if(deviceInfo!==null&&deviceInfo!==undefined){var info=deviceInfo;
...
var analytics="/etc.clientlibs/minecraft/clientlibs/main/resources/scripts/atv221-minecraft.js";
var optimizeLink="https://cdn.optimizely.com/js/6954420415.js";
var execute=false;
$(document).on("click",":button, .btn, .site-body a, .header a",function(){if(getCookie("mbox").length==0){if(!execute){mscc.setConsent();
jQuery.getScript(analytics);
jQuery.getScript(optimizeLink);
execute=true
}}});
var grid=document.querySelector(".grid");
if(grid!=null){grid.onclick=cssGridAnalytics;
function cssGridAnalytics(a){if(a.target.matches(".tile__image, .tile-caption, .tile-category, .tile-caption__text, .tile-caption__title, .tile-category__text")){if(getCookie("mbox").length==0){if(!execute){mscc.setConsent();
jQuery.getScript(analytics);
jQuery.getScript(optimizeLink);
execute=true
}}}}};